...
const PRESETS: Record<NonNullable<Exclude<SpinnerOptions['frames'], string[]>>, string[]> = {
  dots: ['⠋','⠙','⠹','⠸','⠼','⠴','⠦','⠧','⠇','⠏'],
  dots2: ['⣾','⣽','⣻','⢿','⡿','⣟','⣯','⣷'],
  line: ['-','\\','|','/'],
  pipe: ['┤','┘','┴','└','├','┌','┬','┐'],
...
export function createSpinner(textOrOptions?: string | SpinnerOptions, colors?: Colors): Spinner {
  let interval: NodeJS.Timeout | null = null;
  let frame = 0;

  let options: SpinnerOptions = typeof textOrOptions === 'string' ? { text: textOrOptions } : (textOrOptions ?? {});
  ...
  function asColor(fn?: (t: string) => string) { return fn ?? ((s: string) => s); }

  function render() {
    const raw: string = (frames[frame % frames.length] ?? frames[0] ?? '') as string;
  ...
  const api: Spinner = {
    ...
    succeed(msg?: string) {
      ...
      const iconColor = options.succeedColor ?? (colors ? colors.green : undefined);
    ...
    setText(text: string) { options.text = text; return this; },
    setColor(color?: (text: string) => string) { options.color = color; return this; },
