...
describe('Subcommands', () => {
  ...
  it('creates nested subcommands', async () => {
    ...
    await withMockedIO(async () => {
      ...
      build.command('web', 'Build web app')
        .argument('<target>')
        .action((args) => {
          outputs.push(`web:${args.target}`);
        });
      
      build.command('api', 'Build API')
        .argument('<version>')
        .action((args) => {
  ...
  it('creates deeply nested subcommands', async () => {
    ...
    await withMockedIO(async () => {
      ...
      web.command('frontend', 'Frontend build')
        .argument('<framework>')
        .action((args) => {
  ...
  it('handles subcommand with both inherited and local middleware', async () => {
    ...
    expect(outputs).toEqual(['root-middleware', 'build-middleware', 'build:web']);
  });
  ...
  it('handles subcommand with mixed option types', async () => {
    ...
    await withMockedIO(async () => {
      ...
      app.command('build', 'Build command')
        ...
        .action((args, options) => {
          outputs.push(`build:${args.target}:${options.prod}:${options.config}:${options.verbose}`);
        });

      await app.parse(['build', 'web', '--prod', '--config', 'prod.json', '--verbose']);
    });
  ...
  it('handles subcommand with default option values', async () => {
    ...
    await withMockedIO(async () => {
      ...
      app.command('build', 'Build command')
        ...
        .option('--config', 'Config file', { defaultValue: 'default.json', valueName: '<config>' })
        .action((args, options) => {
          outputs.push(`build:${args.target}:${options.prod}:${options.config}`);
        });
  ...
  it('handles subcommand with complex command path', async () => {
    ...
    await withMockedIO(async () => {
      ...
      frontend.command('react', 'React build')
        .argument('<version>')
        .action((args) => {
  ...
  it('handles subcommand with no description', async () => {
    ...
    const app = cli({ name: 'app' });
    app.command('build') // No description
      .action(() => {});
  ...
  it('handles subcommand with empty description', async () => {
    ...
    const app = cli({ name: 'app' });
    app.command('build', '') // Empty description
      .action(() => {});
